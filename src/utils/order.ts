...
export const getEquipmentOnOwnerIds = (item: CartItem, quantity: number) => {
  ...
  const owners = sortByOwnerPriority(item.owner.filter((owner) => !owner.deleted));
...
export const getOrderEquipmentOnOwners = (equipments: EquipmentOwner[], bookId: string) => {
...
export const calculateOwnerEarning = (newOrder: NewOrder, applyToSub?: boolean) => {
    ...
    if (quantity) {
      ...
      if (ownerName === "Federico") {
        ...
        if (!applyToSub && discount) {
          ...
          const bothEarningsWithDiscount = bothEarnings - bothEarnings * (discount.rule.value / 100);
          const bothEarningsWithSubDiscount = bothEarningsWithDiscount - cleanPrice * (discount.rule.value / 100);
