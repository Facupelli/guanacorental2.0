...
import { getEquipmentOnOwnerIds, getOrderEquipmentOnOwners } from "~/utils/order";
import { ADMIN_ORDERS_SORT, ORDER_DELIVER_STATUS, ORDER_RETURN_STATUS } from "~/lib/constants";
import { type Prisma } from "@prisma/client";
import { formatPrice, isEquipmentAvailable } from "~/lib/utils";
import { calcualteAndCreateEarnings, getUpdatedCart, updateEarnings } from "~/utils/updateOrder";
import dayjs from "dayjs";
import { sendCancelOrderMail, sendMail, sendMailToGuanaco } from "~/utils/mailer";
...
export const orderRouter = createTRPCRouter({
  ...
  addEquipmentToOrder: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const equipmentOnOwnerIds = updatedCart.map((item) => getEquipmentOnOwnerIds(item, item.quantity)).flat();
  ...
  getOrderById: protectedProcedure.input(z.object({ orderId: z.string() })).query(async ({ input }) => {
    const { orderId } = input;
  ...
  getCalendarOrders: protectedProcedure.input(z.object({ location: z.string() })).query(async ({ input }) => {
  ...
  setOrderReturned: protectedProcedure.input(z.object({ orderId: z.string() })).mutation(async ({ input }) => {
    const { orderId } = input;
